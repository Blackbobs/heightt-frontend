...
import { Bell, ChevronDown, Settings, LogOut, User as UserIcon, CheckCircle } from "lucide-react";
import { User } from "@/lib/api/users";
import { useAuthStore } from "@/store/auth-store";
import { useUnreadNotificationCount, useMarkAllNotificationsAsRead } from "@/hooks/queries/useCommunication";
...
export function DashboardHeader({
  ...
  const { data: unreadCount = 0, isLoading: isLoadingUnread } = useUnreadNotificationCount();
  ...
  const handleNotificationClick = () => {
    if (onNotificationClick) {
      ...
      // Navigate to notifications page or toggle dropdown
  ...
  const getUserRole = () => {
    ...
    // This would come from your user roles - adjust as needed
            ...
            <div className="absolute right-0 top-[calc(100%+8px)] min-w-[220px] bg-white rounded-lg shadow-[0_4px_16px_rgba(0,0,0,0.12)] border border-[#e8ecf1] py-1.5 z-50">
              ...
              <div className="px-4 py-2.5 border-b border-[#e8ecf1]">
                <div className="flex items-center gap-2.5">
                  <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${getAvatarColor()} flex items-center justify-center text-white font-semibold text-[0.7rem]`}>
                    {getInitials()}
                  </div>
              ...
              <div className="py-1">
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-[#1a1a2e] hover:bg-[#f0f2f5] transition-colors"
                >
                ...
                <div className="border-t border-[#e8ecf1] my-1" />

                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-[#ef4444] hover:bg-[#fef2f2] transition-colors"
                >
