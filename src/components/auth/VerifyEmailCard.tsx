...
import { Mail, RotateCcw, ArrowRight, CheckCircle2, Sparkles } from 'lucide-react';
...
export function VerifyEmailCard() {
  ...
  useEffect(() => {
    if (digits.every((d) => d !== '') && !isVerified && !isVerifying) {
      handleVerify();
    }
  }, [digits, handleVerify, isVerified, isVerifying]);
  ...
  /* ── Render ──────────────────────────────────────────────────── */
  return (
    <div className="w-full max-w-[520px]" role="main" aria-labelledby="verify-heading">
      {/* Icon badge */}
      ...
      <p className="text-[0.95rem] text-[#5b6d89] mb-7 font-normal leading-snug">
        {isVerified
          ...
          : <>We sent a 6-digit code to&nbsp;<strong className="text-[#1a5cff] font-semibold">{email}</strong>.</>}
      </p>
        ...
        <>
          ...
          <div className="mt-6 text-center text-[0.88rem] text-[#5b6d89]">
            {canResend ? (
              ...
              <span>
                Resend code in{' '}
                <strong className="text-[#1a5cff] tabular-nums">{countdown}s</strong>
              </span>
            )}
          </div>
