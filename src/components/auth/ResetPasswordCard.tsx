...
export function ResetPasswordCard() {
  ...
  return (
    <div className="w-full bg-white sm:bg-transparent p-6 sm:p-0 rounded-3xl sm:rounded-none border border-slate-200/90 sm:border-none shadow-xl shadow-slate-900/5 sm:shadow-none" role="main" aria-labelledby="reset-password-heading">
      ...
      <h1 id="reset-password-heading" className="text-2xl sm:text-[1.6rem] font-bold text-[#0B1020] tracking-tight mb-1.5">{success ? 'Password updated' : 'Create a new password'}</h1>
      <p className="text-[0.92rem] text-[#64748B] mb-6 leading-relaxed">{success ? 'Your password was changed and your existing sessions were signed out.' : 'Choose a new password for your Heightt account.'}</p>
      ...
      {success ? (
        <div className="space-y-5">
          ...
          <Link href="/signin" className="flex w-full items-center justify-center rounded-xl px-5 py-3.5 text-base font-semibold text-white bg-[#2563EB] hover:bg-[#1D4ED8] no-underline">Sign in</Link>
        </div>
      ) : token ? (
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          <button type="submit" disabled={submitting} className="w-full border-none rounded-xl px-5 py-3.5 sm:py-4 text-base font-semibold text-white bg-[#2563EB] hover:bg-[#1D4ED8] shadow-[0_8px_24px_rgba(26,92,255,0.25)] disabled:opacity-70 flex items-center justify-center gap-2">
        ...
        <div className="space-y-5">
          ...
          <Link href="/forgot-password" className="flex w-full items-center justify-center rounded-xl px-5 py-3.5 text-base font-semibold text-white bg-[#2563EB] hover:bg-[#1D4ED8] no-underline">Request a new link</Link>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="relative mt-1.5">
        ...
        <input id={id} name={id} type={visible ? 'text' : 'password'} autoComplete="new-password" required minLength={8} maxLength={128} value={value} onChange={(event) => setValue(event.target.value)} className="w-full bg-white sm:bg-[#F8FAFC] border-[1.5px] border-[#cbd5e1] rounded-xl pl-10 pr-11 py-3 text-[0.95rem] font-medium text-[#0B1020] outline-none focus:border-[#2563EB] focus:bg-white focus:ring-4 focus:ring-[#2563EB]/10" />
